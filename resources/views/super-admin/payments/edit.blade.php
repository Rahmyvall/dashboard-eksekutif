          ...
          <div class="pay-edit-wrap">
               <header class="pay-hero">
                    <div class="pay-hero-row">
                         <div class="pay-heading"><span class="pay-icon"><i class="fas fa-pen-to-square"></i></span>
                              <div>
                                   <h1>Edit Pembayaran</h1>
                                   <p>{{ $payment->payment_number }} · Perbarui data transaksi dengan teliti.</p>
                              </div>
                         </div><a class="pay-back" href="{{ route('super-admin.payments.show', $payment) }}"><i
                                   class="fas fa-arrow-left"></i> Kembali ke Detail</a>
                    </div>
               </header>
               <div class="pay-layout">
                    <main class="pay-card">
                         ...
                         @endif
                         <form method="POST" action="{{ route('super-admin.payments.update', $payment) }}"
                              enctype="multipart/form-data">@csrf @method('PUT')
                              <section class="pay-section">
                                   <h2 class="pay-section-title"><i class="fas fa-link"></i> Relasi Transaksi</h2>
                              </section>
                              <div class="row g-3 mb-4">
                                   <div class="col-md-6"><label class="pay-label" for="service_order_id">Service Order
                                             ...
                                             @foreach ($payment->serviceOrder ? [$payment->serviceOrder] : [] as $order)
                                                  <option value="{{ $order->id }}" selected>{{ $order->order_number }} —
                                                       {{ $order->customer?->display_name ?? $order->customer?->name }}
                                                  </option>
                                             @endforeach
                                        </select>
                                        <small class="pay-help">Service order menjadi sumber validasi invoice dan
                                             tagihan.</small>
                                   </div>
                                   <div class="col-md-6"><label class="pay-label" for="invoice_id">Invoice <span
                                             ...
                                             @foreach ($invoices as $invoice)
                                                  <option value="{{ $invoice->id }}"
                                                       ...
                                                       @selected(old('invoice_id', $payment->invoice_id) == $invoice->id)>{{ $invoice->invoice_number }} —
                                                       {{ $invoice->serviceOrder?->order_number }}</option>
                                             @endforeach
                                        </select>
                                        <small class="pay-help">Invoice yang dipilih harus milik service order di sebelah
                                             kiri.</small>
                                   </div>
                              </div>
                              <section class="pay-section">
                              ...
                              <div class="row g-3 mb-4">
                                   ...
                                   <div class="col-md-4"><label class="pay-label" for="payment_method">Metode Pembayaran
                                             ...
                                             @foreach ($methods as $method)
                                                  <option value="{{ $method }}" @selected(old('payment_method', $payment->payment_method) === $method)>
                                   ...
                                   <div class="col-md-4"><label class="pay-label" for="amount">Jumlah Pembayaran
                                        ...
                                        <div class="input-group"><span
                                                  ...
                                                  value="{{ old('amount', $payment->amount) }}" required></div>
                                   </div>
                                   <div class="col-md-6"><label class="pay-label" for="reference_number">Nomor
                                             Referensi</label><input class="form-control pay-control readonly-reference"
                                             id="reference_number"
                                             value="{{ $payment->reference_number ?: 'Akan dibuat otomatis' }}"
                                             readonly><small class="pay-help">Nomor referensi tidak dapat diubah
                                             manual.</small></div>
                              ...
                              <div class="row g-3">
                                   <div class="col-md-6"><label class="pay-label" for="proof_of_payment">Ganti Bukti
                                             Pembayaran <span class="required-note">Opsional</span></label><label
                                             class="pay-upload" for="proof_of_payment"><input type="file"
                                                  id="proof_of_payment" name="proof_of_payment"
                                                  accept=".jpg,.jpeg,.png,.pdf"><span><span class="upload-icon"><i
                                                            class="fas fa-cloud-arrow-up"></i></span><span
                                                       class="upload-title" id="uploadTitle">Pilih bukti pembayaran
                                                       baru</span><span class="upload-subtitle" id="uploadSubtitle">JPG,
                                                       PNG, atau PDF · Maksimal 5 MB</span></span></label>
                                        @if ($payment->proof_of_payment_path)
                                             <a class="current-proof" target="_blank"
                                                  href="{{ asset('storage/' . $payment->proof_of_payment_path) }}"><i
                                                       class="fas fa-paperclip"></i> Lihat bukti saat ini</a>
                                        @endif
                                   </div>
                                   <div class="col-md-6"><label class="pay-label" for="notes">Catatan <span
                                                  class="required-note">Opsional</span></label>
                                        <textarea class="form-control pay-control" id="notes" name="notes" rows="5">{{ old('notes', $payment->notes) }}</textarea>
                                   </div>
                              </div>
                              <div class="pay-actions"><a class="pay-cancel"
                                        href="{{ route('super-admin.payments.show', $payment) }}">Batal</a><button
                                        class="pay-submit" type="submit"><i class="fas fa-save"></i> Simpan
                    ...
                    <aside class="pay-side">
                         <div class="pay-side-card">
                              ...
                              <div class="pay-preview"><span class="preview-label">Total diterima</span>
                                   ...
                                   <div class="preview-meta"><span id="methodPreview">-</span><span class="preview-status"
                                             id="statusPreview">-</span></div>
                              </div>
                              <div class="pay-stat">
                                   <span>Referensi</span><strong>{{ $payment->reference_number ?: 'Otomatis' }}</strong>
                              </div>
                              <div class="pay-stat"><span>Tanggal</span><strong id="datePreview">-</strong></div>
                              <div class="pay-stat"><span>Service
                                        Order</span><strong>{{ $payment->serviceOrder?->order_number ?? '-' }}</strong>
                              </div>
                         </div>
                         <div class="pay-side-card">
                              <h3 class="pay-side-title"><i class="fas fa-shield-halved"></i> Catatan Perubahan</h3>
                              <div class="pay-stat"><span>Nomor
                                        payment</span><strong>{{ $payment->payment_number }}</strong></div>
                              <div class="pay-stat">
                                   <span>Dibuat</span><strong>{{ $payment->created_at?->format('d/m/Y') ?? '-' }}</strong>
                              </div>
                              <ul class="mt-3 mb-0 ps-3 text-muted" style="font-size:.8rem;line-height:1.6">
                                   <li>Nomor referensi dikunci oleh sistem.</li>
                                   <li>Invoice harus sesuai service order.</li>
                                   <li>Nominal divalidasi terhadap sisa tagihan.</li>
                              </ul>
                         </div>
                    </aside>
               </div>
          ...
          (function() {
               ...
               const filterInvoices = () => {
                    ...
                    Array.from(invoice.options).forEach(option => {
                         ...
                         if (option.value === invoice.value) selected = visible;
                    });
                    if (!selected) invoice.value = '';
               };
               [amount, method, status, date].forEach(el => el.addEventListener('input', refresh));
