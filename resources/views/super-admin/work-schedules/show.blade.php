...
@section('content')
     @php
          /*
           * Mendukung beberapa nama variabel dari controller lama,
           * tetapi nama yang disarankan tetap $workSchedule.
           */
          ...
          $formatScheduleTime = static function ($value) {
               if (blank($value)) {
                    return '—';
               }
          ...
          if ($workSchedule) {
               $workingHours = is_numeric($workSchedule->working_hours)
                    ? rtrim(
                         rtrim(
                              number_format(
                                   (float) $workSchedule->working_hours,
                                   2,
                                   ',',
                                   '.'
                              ),
               ...
               <section class="position-show-hero">
                    <div class="position-show-hero-content">
                         ...
                         <div class="position-show-hero-actions">
                              <a
                                   href="{{ route('super-admin.work-schedules.index') }}"
                                   class="position-show-action"
                              >
                              ...
                              @if (Route::has('super-admin.work-schedules.edit'))
                                   <a
                                        href="{{ route('super-admin.work-schedules.edit', $workSchedule) }}"
     ...
     @else
          ...
          ">
               <strong style="display:block;margin-bottom:8px;">
                    Data jadwal kerja tidak diterima oleh view.
               </strong>

               Pastikan method <code>show()</code> pada WorkScheduleController
               mengirim variabel <code>$workSchedule</code> menggunakan
               <code>compact('workSchedule')</code>.
          </div>
     @endif
